...
def powerset(iterable):
    """Return all possible subsets (the powerset) of a list."""
    s = list(iterable)
    # chain.from_iterable creates sequences of subsets of each length
...
# ):
#     """
#     Bidirectional stepwise feature selection for your custom sklearn Pipeline.
#     Stops when no significant CV improvement occurs (min_delta).
#     """
...
#                 improved = True
#                 if verbose:
#                     print(f"➕ Added '{best_candidate}' → mean CV={best_score:.4f} (+{delta:.4f})")
...
#                 improved = True
#                 if verbose:
#                     print(f"➖ Removed '{worst_candidate}' → mean CV={best_score:.4f} (+{delta:.4f})")

#         history.append((iteration + 1, len(selected), best_score))

#         if not improved:
#             if verbose:
#                 print(f"\n⏹️ No significant improvement after iteration {iteration+1}. Stopping.")
#             break

#     return selected, pd.DataFrame(history, columns=["iteration", "n_features", "cv_accuracy"])
def backward_selection_to_n(
    X, y,
    base_pipe,
    target_n=15,
    cv=5,
    ...
    """
    Backward elimination: starts from all features and removes the least useful ones
    until only `target_n` features remain.
    Evaluates each removal using cross-validation accuracy.
    """
    kfold = KFold(n_splits=cv, shuffle=True, random_state=42)
    selected = list(X.columns)
    history = []

    # Compute initial baseline score
    ...
    while len(selected) > target_n:
        ...
        # Find feature whose removal gives the highest score (or least loss)
        worst_candidate, worst_candidate_score = max(scores_with_candidates, key=lambda x: x[1])
        delta = worst_candidate_score - best_score
...
# PCA?
# Usa il tuo modello definito sopra
USE_PCA = False
POLY_ENABLED = False
...
start = time.time()
selected_features, history = backward_selection_to_n(
    X=train_df[features],
    y=train_df["player_won"],
    base_pipe=pipe,
    target_n=15,
...
print(f"\n⏱️ Elapsed time: {elapsed:.2f} seconds ({elapsed/60:.2f} min)")

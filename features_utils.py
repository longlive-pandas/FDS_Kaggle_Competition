...
def get_net_stat_boost_advantage(timeline):
    """
    Calculates the cumulative net stat boost advantage for Player 1 over Player 2 
    across all turns in the battle timeline.
    
    A positive result means P1 benefited more from stat boosts/drops than P2.
    """
    if not timeline:
        return 0.0
    ...
    # We only care about the change in boosts, not the base stats
    ...
    for turn in timeline:
        ...
        # Calculate the total boost stage for the active Pokémon for each player
        p1_total_boost = 0
        for key in stat_keys:
            # Boosts are typically integers (-6 to +6)
            p1_total_boost += p1_boosts.get(key, 0)

        p2_total_boost = 0
        for key in stat_keys:
            p2_total_boost += p2_boosts.get(key, 0)
            
        # The advantage for P1 in this turn is the difference
        turn_boost_diff = p1_total_boost - p2_total_boost
        
        # Store the difference for every turn where both Pokémon were active
        stat_boost_diffs.append(turn_boost_diff)

    if not stat_boost_diffs:
        return 0.0
    
    # Return the average difference across the battle. 
    # Using the sum (cumulative) or the mean is a design choice; 
    # using the mean makes it comparable across battles of different lengths.
    return np.mean(stat_boost_diffs)

# The name of the resulting feature is the function's goal:
...
def compute_avg_type_advantage_over_timeline(timeline, pokemon_dict, type_chart, is_test=False, battle_id=''):
    ...
    for turn in timeline:
        ...
        # Get types for defense
        p1_def_types = pokemon_dict.get(p1_name.lower(), [])
        p2_def_types = pokemon_dict.get(p2_name.lower(), [])

        # --- P1 attacking P2 (Based on Move Type) ---
        p1_move_details = turn.get("p1_move_details")
        p1_move_type = None
        if p1_move_details:
            p1_move_type = p1_move_details.get("type")
        p1_adv = 1.0 # Default multiplier

        if p1_move_type:
            p1_adv = 1.0
            for def_type in p2_def_types:
                # Use the actual move type against the defender's types
                mult = type_chart.get(p1_move_type.title(), {}).get(def_type.title(), 1.0)
                p1_adv *= mult
        # If no move details (e.g., switched or skipped turn), use the current 1.0
        # If the move was a status move, p1_adv remains 1.0, which is generally fine

        # --- P2 attacking P1 (Based on Move Type) ---
        p2_move_details = turn.get("p2_move_details")
        p2_move_type = None
        if p2_move_details:
            p2_move_type = p2_move_details.get("type")
        p2_adv = 1.0 # Default multiplier

        if p2_move_type:
            p2_adv = 1.0
            for def_type in p1_def_types:
                # Use the actual move type against the defender's types
                mult = type_chart.get(p2_move_type.title(), {}).get(def_type.title(), 1.0)
                p2_adv *= mult
    ...
    #timeline summary
    p1_avg = np.sum(p1_advantages)
    p2_avg = np.sum(p2_advantages)
    return {
...
def create_features(data: list[dict], is_test=False) -> pd.DataFrame:
    ...
    for battle in tqdm(data, desc="Extracting features"):
        ...
        if timeline:
            ...
            net_stat_boost_advantage = get_net_stat_boost_advantage(timeline)
            features['net_stat_boost_advantage'] = net_stat_boost_advantage
